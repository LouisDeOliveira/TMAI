...
class DDPG_trainer:
    ...
    def fill_buffer(self):
        while len(self.buffer) < 2*self.batch_size:
            episode = play_episode(self.model, self.env, act_value=lambda : [1.0, np.random.uniform(-1.0, 1.0)])
    ...
    def optimization_step(self):
        ...
        state_batch = torch.tensor(np.array([t.state for t in batch]), dtype=torch.float).to(self.device)
        action_batch = torch.tensor(np.array([t.action for t in batch]), dtype=torch.int64).to(self.device)
        reward_batch = torch.tensor(np.array([t.reward for t in batch]), dtype=torch.float).to(self.device)
        next_state_batch = torch.tensor(np.array([t.next_state for t in batch]), dtype=torch.float).to(self.device)
        print("reward_batch", reward_batch.shape)
        next_q = self.target.value(next_state_batch, self.target.policy(next_state_batch))
        ...
        policy_loss = -self.model.value(state_batch, self.model.policy(state_batch).squeeze()).mean()
    ...
    def update_target(self):
        for target_param, param in zip(self.target.policy.parameters(), self.model.policy.parameters()):
            target_param.data.copy_(self.tau * param.data + (1.0 - self.tau) * target_param.data)

        for target_param, param in zip(self.target.value.parameters(), self.model.value.parameters()):
            target_param.data.copy_(self.tau * param.data + (1.0 - self.tau) * target_param.data)
