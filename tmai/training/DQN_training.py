...
class DQN_trainer:
    ...
    def __init__(self, batch_size = 32, N_epochs = 100):
        ...
        self.agent = EpsilonGreedyDQN(input_size=self.env.observation_space.shape[0], device=self.device)
    ...
    def optimze_step(self):
        ...
        action_batch = torch.tensor(np.array([inv_map[tuple(t.action)] for t in batch]), dtype=torch.int64).to(self.device)
        ...
        state_action_values = self.agent.policy(state_batch).gather(1, action_batch.unsqueeze(1)).squeeze(1)
