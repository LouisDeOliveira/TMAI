...
class EpsilonGreedyDQN(Agent):
    ...
    i + 2*j +4*k +8*l:[i, j, k ,l] for i in range(2) for j in range(2) for k in range(2) for l in range(2)
}
    ...
    def epsilon(self):
        return self.eps_end + (self.eps_start - self.eps_end) * np.exp(-1. * self.step /self.eps_decay)
    
    def act(self, observation):
        if np.random.rand() < self.epsilon():
            self.step+=1
            return self.action_correspondance[np.argmax(self.policy(observation).detach().cpu().numpy())]
        self.step+=1
        return self.action_correspondance[np.random.randint(0, len(self.action_correspondance))]
