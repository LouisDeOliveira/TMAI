...
class TMNFClient(Client):
    def __init__(self, action_freq = 100, verbose = True, env:TMNFEnv = None, policy:"Agent" = None, value:"Value" = None):
        super().__init__()
    ...
    def train_step(self,):
        ...
        states = torch.cat([ToTensor()(trans.state).float().unsqueeze(0) for trans in transitions], dim = 0).float().cuda()
        actions = torch.tensor(np.array([trans.action for trans in transitions]), device=DEVICE, dtype = torch.float)
        rewards = torch.tensor(np.array([trans.reward for trans in transitions]), device=DEVICE, dtype = torch.float)
        next_states  = torch.cat([ToTensor()(trans.next_state).float().unsqueeze(0) for trans in transitions], dim = 0).float().cuda()
